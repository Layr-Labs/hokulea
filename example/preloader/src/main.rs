...
pub async fn prepare_witness<O, Evm>(
    ...
    evm_factory: Evm,
    canoe_provider: impl CanoeProvider,
) -> anyhow::Result<EigenDAPreimageWitness>
where
    ...
    <Evm as EvmFactory>::Tx: FromTxWithEncoded<OpTxEnvelope> + FromRecoveredTx<OpTxEnvelope>,
{
    ...
    let mut wit: EigenDAPreimageWitness =
...
/// This client uses a special [OracleEigenDAWitnessProvider] that wraps around [OracleEigenDAPreimageProvider]
...
pub async fn run_witgen_client<O, Evm>(
    oracle: Arc<O>,
    evm_factory: Evm,
) -> Result<EigenDAPreimageWitness, FaultProofProgramError>
where
    O: CommsClient + FlushableCache + Send + Sync + Debug,
    Evm: EvmFactory<Spec = OpSpecId> + Send + Sync + Debug + Clone + 'static,
    <Evm as EvmFactory>::Tx: FromTxWithEncoded<OpTxEnvelope> + FromRecoveredTx<OpTxEnvelope>,
{
    ...
    let eigenda_preimage_witness = Arc::new(Mutex::new(EigenDAPreimageWitness::default()));

    let eigenda_blob_and_witness_provider = OracleEigenDAWitnessProvider {
        provider: eigenda_preimage_provider,
        witness: eigenda_preimage_witness.clone(),
    };
    ...
    let wit = core::mem::take(eigenda_preimage_witness.lock().unwrap().deref_mut());

    Ok(wit)
}

// By this time,both Oracle and EigenDAPreimageWitness are generated by some party that runs run_witgen_client.
// This party now needs to send both of them as inputs to ZKVM. So imagine wit and oracle are sent away, and
// the code region below are some codes that runs inside ZKVM. The ZKVM will convert EigenDAPreimageWitness into

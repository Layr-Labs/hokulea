...
fn main() {
    ...
    let batch_header = BatchHeaderV2::abi_decode(&batch_header_abi, true).unwrap();
    let blob_inclusion_info = BlobInclusionInfo::abi_decode(&blob_inclusion_info_abi, true).unwrap();
    let non_signer_stakes_and_signature = NonSignerStakesAndSignature::abi_decode(&non_signer_stakes_and_signature_abi, true).unwrap();

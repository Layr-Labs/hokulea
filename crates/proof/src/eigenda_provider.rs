...
impl<T: CommsClient + Sync + Send> EigenDABlobProvider for OracleEigenDAProvider<T> {
    ...
    async fn get_blob(&mut self, meta_data: [u8; 4], cert: &BlobInfo) -> Result<Blob, Self::Error> {
        ...
        let mut blob: Vec<u8> =
            vec![0; blob_length as usize * BYTES_PER_FIELD_ELEMENT];
        let field_element_key = get_eigenda_field_element_key_part(&proxy_request_key);
        fetch_blob(field_element_key, blob_length, self.oracle.clone(), &mut blob).await?;
        
        Ok(blob.into())
    }

    /// get_blob_v2 takes a v2 cert type as opposed to bytes stream
    async fn get_blob_v2(&mut self, meta_data: [u8; 4], cert: &EigenDAV2Cert) -> Result<Blob, Self::Error> {
        let mut cert_rlp_bytes = Vec::<u8>::new();
        // rlp encode of cert
        cert.encode(&mut cert_rlp_bytes);
...
async fn fetch_blob<T: CommsClient>(mut field_element_key: [u8; 80], blob_length: u64, oracle: Arc<T>, blob: &mut [u8]) -> Result<(), OracleProviderError>{

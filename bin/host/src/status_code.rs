...
pub const STATUS_CODE_INVALID_CERT_OR_INCONSISTENT_OFFCHAIN_DERIVATIOM_ERROR: u8 = 3;
...
impl From<DerivationError> for HostHandlerError {
    fn from(status: DerivationError) -> Self {
        match status.status_code {
            STATUS_CODE_INVALID_CERT_OR_INCONSISTENT_OFFCHAIN_DERIVATIOM_ERROR => {
                HostHandlerError::HokuleaPreimageError(
